...
namespace R34Downloader
{
    ...
    {
        public static string MainRequestPath 
        {
            get
            {
        ...
        public static void DownloadPosts(List<string> postList, string path, int pid, int residue, int maxPages, IProgress<int> progress, IProgress<int> progress2)
        {
            ...
            for (int i = 0; i < maxPosts; i++)
            {
                ...
                if (videoNode != null && Transfer.Video)
                {
                    ...
                    DownloadContent("https:" + videoPath, path + "\\Video\\" + postList.ElementAt(i).Split('=')[3] + "." + videoPath.Split('.')[2]);
                }
                else if (imageNode != null)
                {
                    ...
                    string filename = id + "." + imagePath.Split('.')[3];

                    if (imagePath.Contains(".gif") && Transfer.Gif)
                    {
                        DownloadContent("https:" + imagePath, path + "\\Gif\\" + filename);
                    }
                    else if (!imagePath.Contains(".gif") && Transfer.Images)
                    {
                        DownloadContent("https:" + imagePath, path + "\\Images\\" + filename);
                    }
                }
